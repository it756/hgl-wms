-- Migration: 044_process_issuance_multi_destination.sql
-- Purpose: A transfer request line item now carries its own destination_unit_id
--          (see 041_multi_destination_transfers.sql), so process_issuance must
--          record which destination each issued line is bound for and verify
--          the (product, destination) pair is actually part of the request.
...
BEGIN
  ...
  -- Process each line item from JSON array {product_id, quantity_issued, destination_unit_id}
  FOR v_item IN
    SELECT * FROM json_to_recordset(p_items)
      AS (product_id uuid, quantity_issued integer, destination_unit_id uuid)
  LOOP
    IF v_item.destination_unit_id IS NULL THEN
      RAISE EXCEPTION 'process_issuance: destination_unit_id is required for product %', v_item.product_id;
    END IF;

    -- Confirm this (product, destination) pair is actually on the request —
    -- a request may fan out to several destination units per product.
    IF NOT EXISTS (
      SELECT 1 FROM transfer_line_items tli
      WHERE tli.transfer_request_id = p_transfer_request_id
        AND tli.product_id = v_item.product_id
        AND tli.destination_unit_id = v_item.destination_unit_id
    ) THEN
      RAISE EXCEPTION 'process_issuance: product % / destination % is not on this transfer request',
        v_item.product_id, v_item.destination_unit_id;
    END IF;
    ...
    -- Insert line item
    INSERT INTO issuance_line_items(issuance_id, product_id, quantity_issued, destination_unit_id, created_at, updated_at)
    VALUES (v_issuance_id, v_item.product_id, v_item.quantity_issued, v_item.destination_unit_id, now(), now());
  END LOOP;
