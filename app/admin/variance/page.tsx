    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm ml-9">
            ...
            proposal review. The Finance Manager decides per-line whether to write off the shortage
            as damage or reintegrate the excess into stock.
          </p>
        </div>
        <button
          onClick={load}
      ...
      <div className="mb-6 flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-xs text-blue-700">
        ...
        <span>
          ...
          <strong>COMPLETED</strong>. Damage write-offs are posted to the Loss Account; excess stock
          is reintegrated back into inventory.
        </span>
      </div>
                      ...
                      <table className="min-w-full divide-y divide-slate-100 text-xs">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {grn.grn_line_items.map((li) => {
                            ...
                            const valueGap =
                              li.products?.unit_cost != null ? delta * li.products.unit_cost : null;

                            return (
                              <tr key={li.id} className={delta !== 0 ? "bg-amber-50/40" : ""}>
                                <Td
                                  pinned
                                  className={`max-w-50 ${delta !== 0 ? "bg-amber-50" : "bg-white"}`}
                                >
                                  <p
                                    className="font-bold text-slate-800 truncate"
                                    title={li.products?.name ?? "Unknown"}
                                  >
