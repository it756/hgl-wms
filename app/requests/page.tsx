...
import { Search, Plus, Calendar, Filter, Eye, XCircle } from "lucide-react";
      ...
      <div className="flex flex-col gap-6 w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Manage and track internal warehouse inventory transfers across SBUs.</p>
          </div>
          <Link
            href="/requests/new"
        ...
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-5 flex flex-wrap items-center gap-4 shadow-sm">
          <div className="flex-1 min-w-[200px]">
            <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">Search by Reference</label>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left">
                <thead className="bg-[#eff4ff] border-b border-outline-variant">
                  <tr>
                    <th className="px-6 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider">Reference</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider">Status</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider">Required Date</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider">Est. Value</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider">Date Raised</th>
                  ...
                  {filteredRequests.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <span className="font-bold text-primary font-mono text-sm leading-none">{r.reference_number}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-bold ${STATUS_COLORS[r.status] ?? "bg-slate-100 text-slate-600 border border-slate-200"}`}>
                      ...
                      <td className="px-6 py-4 text-sm font-semibold text-slate-500">
                        {r.required_date ? new Date(r.required_date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "—"}
                      </td>
                      <td className="px-6 py-4 text-sm font-mono font-bold text-slate-600">
                        {r.estimated_value != null ? `KES ${r.estimated_value.toLocaleString()}` : "—"}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                          <button className="p-1.5 hover:bg-slate-100 rounded text-slate-600 cursor-pointer" title="View Details">
