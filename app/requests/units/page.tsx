...
export default function BUUnitsPage() {
  ...
  const [staffRequestMessage, setStaffRequestMessage] = useState<string | null>(null);
  ...
  async function handleInviteStaff(e: React.FormEvent) {
    ...
    try {
      ...
      setNewStaffEmail("");
      setNewStaffUnitId("");
      setStaffRequestMessage(data.message ?? "Staff creation request submitted.");
    } catch (e: any) {
                      ...
                      <td className="px-5 py-3.5">
                        {person.role === "BU_MANAGER" ? (
                          ...
                          <span className="text-sm text-slate-600">
                            {person.sbu_units
                              ? `${person.sbu_units.name} (${person.sbu_units.code})`
                              : "— Unassigned —"}
                          </span>
                        )}
                      </td>
